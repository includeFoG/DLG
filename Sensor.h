...
#include "Config.h"   //usado para powerManager
...
class Sensor { //clase base para todos los tipos de sensor, abstracta
  ...
  public:
    ...
    Sensor(std::string nombre, bool poweredByBoard, std::optional<int> pinRelated);
...
class sensRS485Modbus : public Sensor {
  ...
  public:
    ...
    //METODOS virtuales:
    void takeMeasure(bool save = true) override; //no es const por node
};
